...
const FAKE_NOTIFS = [
  ...
  { icon: '🏎️', text: 'LIVE: Qualifiche F1 Monaco — VER in POLE' },
...
const NEWS_ITEMS = [
  ...
  '🏎️ F1 Monaco: Verstappen pole position con 0.003s di vantaggio',
  ...
  '📱 Apple annuncia iPhone 20: ora ti legge il pensiero',
  '🧬 UniTo: scoperta cura per l\'ansia da esami (è la laurea)',
...
const ALL_UPGRADES = [
  { id: 'amount_anim',  name: 'Amount Animation',  cost: 10,   desc: 'Animazione sui click!',        spc: 1, sps: 0,  emoji: '✨', unlockAt: 2 },
  { id: 'dvd',          name: 'Bouncing DVD',        cost: 3,    desc: 'Logo DVD rimbalzante! +1 spb', spc: 0, sps: 0,  emoji: '📀', unlockAt: 2, multi: true, spb: 1 },
  { id: 'sps_counter',  name: 'SPS Counter',         cost: 25,   desc: 'Mostra stimolazione/sec',      spc: 0, sps: 0,  emoji: '📊', unlockAt: 5 },
  { id: 'btn_upgrade',  name: 'Button Upgrade',      cost: 50,   desc: 'Bottone più grande e bello!',  spc: 1, sps: 0,  emoji: '🔵', unlockAt: 5 },
  { id: 'subway',       name: 'Subway Surfers',      cost: 100,  desc: '+3 stim/sec in background',    spc: 0, sps: 3,  emoji: '🛹', unlockAt: 10 },
  { id: 'news',         name: 'Breaking News',       cost: 100,  desc: 'Ticker notizie in cima!',      spc: 0, sps: 4,  emoji: '📺', unlockAt: 10 },
  { id: 'critical',     name: 'Critical Hits',       cost: 200,  desc: '5% chance di colpo critico!', spc: 0, sps: 0,  emoji: '💥', unlockAt: 25, needs: 'amount_anim' },
  { id: 'lofi',         name: 'Lofi Beats',          cost: 250,  desc: '+10 stim/sec di relax',        spc: 0, sps: 10, emoji: '🎵', unlockAt: 25 },
  { id: 'rain',         name: 'Rain Sounds',         cost: 300,  desc: '+15 stim/sec di pioggia',      spc: 0, sps: 15, emoji: '🌧️', unlockAt: 50 },
  { id: 'pinwheel',     name: 'Pinwheel',            cost: 700,  desc: '+20 stim/sec di girandola',    spc: 0, sps: 20, emoji: '🌀', unlockAt: 50 },
  { id: 'hydraulic',    name: 'Hydraulic Press',     cost: 1000, desc: '+50 stim ogni schiacciata!',   spc: 0, sps: 0,  emoji: '🔩', unlockAt: 100 },
  { id: 'achieve',      name: 'Achievements',        cost: 1500, desc: 'Sblocca gli achievements!',    spc: 0, sps: 0,  emoji: '🏆', unlockAt: 100 },
  { id: 'slime',        name: 'Slime ASMR',          cost: 4000, desc: '+35 stim/sec ASMR',            spc: 0, sps: 35, emoji: '🫧', unlockAt: 500 },
  { id: 'podcast',      name: 'True Crime Podcast',  cost: 10000,desc: '+100 stim/sec podcast',        spc: 0, sps: 100,emoji: '🎙️', unlockAt: 1000 },
];

const ACHIEVEMENTS_DEF = [
  { id: 'first_click',   name: 'Primo Click!',        desc: 'Hai iniziato il tuo percorso.',   check: (s) => s.totalClicks >= 1 },
  { id: 'click_100',     name: 'Click Cadet',          desc: '100 click sul bottone.',          check: (s) => s.totalClicks >= 100 },
  { id: 'click_500',     name: 'Click Corporal',       desc: '500 click sul bottone.',          check: (s) => s.totalClicks >= 500 },
  { id: 'stim_100',      name: 'Procrastinatore',      desc: '100 stimolazione totale.',        check: (s) => s.totalProduced >= 100 },
  { id: 'stim_1000',     name: 'Brain Rot Iniziale',   desc: '1.000 stimolazione prodotta.',    check: (s) => s.totalProduced >= 1000 },
  { id: 'dvd_bounce',    name: 'Disc Collector',       desc: 'Compra 1 DVD bouncing.',          check: (s) => s.boughtIds.includes('dvd') },
  { id: 'upgrade_5',     name: 'Fixer Upper',          desc: 'Acquista 5 upgrade.',             check: (s) => s.upgradeCount >= 5 },
];

// DVD logo component — faithful black+white rectangle look
function DVDLogo({ color }) {
  return (
    <div style={{
      background: '#000', border: `2px solid ${color}`, borderRadius: 4,
      padding: '3px 7px', display: 'inline-flex', flexDirection: 'column',
      alignItems: 'center', lineHeight: 1, boxShadow: `0 0 10px ${color}`,
    }}>
      <span style={{ color, fontWeight: 900, fontSize: '0.9rem', letterSpacing: '0.15em', fontFamily: 'sans-serif' }}>DVD</span>
      <div style={{ width: '100%', height: 2, background: `linear-gradient(to right, transparent, ${color}, transparent)`, marginTop: 1 }} />
    </div>
  );
}

// Web Audio lofi music hook
function useLofiMusic(enabled) {
  ...
  useEffect(() => {
    ...
    const ctx = new Ctx();
    ctxRef.current = ctx;
    // Lofi chord: Cmaj7 = C E G B (gentle sine waves)
    const freqs = [130.81, 164.81, 196.00, 246.94, 261.63];
    ...
    const nodes = freqs.map((freq, i) => {
      ...
      gain.gain.setValueAtTime(0.15 / freqs.length, ctx.currentTime);
      osc.connect(gain);
      gain.connect(master);
      osc.start();
      return osc;
...
function useDVDs(count, onBounce) {
...
function StimulationClicker({ onClose }) {
  ...
  const [uiUpgrade, setUiUpgrade] = useState(false);
  const nextId = useRef(0);

  // dvd bounce gives +spb stimulation
  const handleDvdBounce = useCallback(() => {
    if (spb <= 0) return;
    setStim(s => s + spb);
    setTotalProd(t => t + spb);
  }, [spb]);

  const dvds = useDVDs(dvdCount, handleDvdBounce);
  useLofiMusic(boughtIds.includes('lofi'));
  ...
  // visible upgrades: unlocked at totalProd threshold, not yet bought (or multi)
  const visibleUpgrades = ALL_UPGRADES.filter(u => {
    if (totalProd < u.unlockAt) return false;
    if (u.needs && !has(u.needs)) return false;
    if (u.multi) return true; // can always re-buy DVD
    return !has(u.id);
  }).slice(0, 5);

  // Keep sps in a ref so the single ticker always reads the latest value
  const spsRef = useRef(0);
  spsRef.current = sps;

  // SPS ticker — ONE interval, runs forever, only adds stim when sps > 0
  useEffect(() => {
    const interval = setInterval(() => {
      if (spsRef.current <= 0) return;          // no upgrades bought → do nothing
      setStim(s => s + spsRef.current / 10);
      setTotalProd(t => t + spsRef.current / 10);
    }, 100);
    return () => clearInterval(interval);
  }, []); // dependency-free: only starts once on mount


  // News ticker scroll
  useEffect(() => {
    if (!hasNews) return;
    const interval = setInterval(() => setNewsOffset(o => (o + 1) % (NEWS_ITEMS.length * 400)), 30);
    return () => clearInterval(interval);
  }, [hasNews]);

  // Hydraulic squish every 8s
  useEffect(() => {
    if (!hasHydraulic) return;
    const interval = setInterval(() => {
  ...
  }, [totalClicks, totalProd, boughtIds, upgradeCount]);

  const addFloater = (x, y, isCrit = false) => {
    const id = nextId.current++;
    const val = isCrit ? Math.floor(spc * (6 + Math.random() * 3)) : spc;
    setFloaters(f => [...f.slice(-30), { id, x, y, val, isCrit }]);
  ...
  const handleClick = (e) => {
    ...
    const gain = isCrit ? Math.floor(spc * (6 + Math.random() * 3)) : spc;
    addFloater(e.clientX - rect.left, e.clientY - rect.top, isCrit);
    setStim(s => s + gain);
    setTotalProd(t => t + gain);
    setTotalClicks(c => c + 1);
  };

  const buyUpgrade = (upg) => {
    if (stim < upg.cost) return;
    setStim(s => s - upg.cost);
    setSpc(s => s + (upg.spc || 0));
    setSps(s => s + (upg.sps || 0));
    if (upg.spb) setSpb(s => s + upg.spb);
    if (upg.id === 'dvd') setDvdCount(c => c + 1);
    if (!upg.multi) setBoughtIds(prev => [...prev, upg.id]);
    setUpgradeCount(c => c + 1);
  };

  const bgStyle = uiUpgrade
    ? { background: 'linear-gradient(135deg, #0f0025 0%, #1a003a 50%, #0a001a 100%)' }
    : { background: '#020817' };

  return (
    <div className="w-screen min-h-screen flex flex-col overflow-hidden relative select-none" style={bgStyle}>

      {/* DVD logos in background */}
      {dvds.map(d => (
      ...
      {hasNews && (
        <div className="fixed top-0 left-0 w-full z-50 bg-red-700 text-white text-xs font-bold py-1 overflow-hidden flex items-center gap-2">
          <span className="bg-white text-red-700 px-2 py-0.5 rounded font-black ml-2 shrink-0">BREAKING</span>
          <div className="overflow-hidden flex-1 relative h-4">
            <div className="whitespace-nowrap absolute" style={{ transform: `translateX(-${newsOffset % (NEWS_ITEMS.join('   ·   ').length * 8)}px)`, transition: 'none' }}>
              {[...NEWS_ITEMS, ...NEWS_ITEMS].join('   ·   ')}
            </div>
          </div>
        </div>
      )}
      ...
      {achieveToast && hasAchieve && (
        <motion.div
          ...
          className="fixed bottom-6 right-4 z-50 bg-yellow-400 text-black rounded-2xl px-4 py-3 shadow-2xl"
        >
          <p className="font-black text-sm">🏆 Achievement!</p>
          <p className="text-xs font-bold">{achieveToast.name}</p>
          <p className="text-xs opacity-70">{achieveToast.desc}</p>
        </motion.div>
      )}
      ...
      <div className="fixed inset-0 pointer-events-none z-30 overflow-hidden">
        {floaters.map(f => (
          <motion.span key={f.id}
            ...
            animate={{ opacity: 0, y: -60, scale: f.isCrit ? 1.8 : 1.2 }}
            transition={{ duration: 0.9, ease: 'easeOut' }}
            style={{ position: 'absolute', left: f.x, top: f.y, color: f.isCrit ? '#ef4444' : '#a78bfa', fontWeight: 900, fontSize: f.isCrit ? '1.4rem' : '1rem' }}
          >
            {f.isCrit ? `💥 CRIT!` : `+${f.val}`}
          </motion.span>
        ))}
      ...
      {/* Hydraulic squish overlay */}
      {hydraulicSquish && (
        <motion.div
          initial={{ scaleY: 0, opacity: 0.8 }} animate={{ scaleY: 1, opacity: 0 }}
          transition={{ duration: 0.6 }}
          className="fixed inset-0 z-40 pointer-events-none bg-slate-300/30"
          style={{ transformOrigin: 'top' }}
        />
      )}

      {/* Navbar */}
      <div className={`relative z-40 pointer-events-auto ${hasNews ? 'mt-6' : ''}`}>
        <Navbar onAction={onClose} />
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-0 relative z-10">
        ...
        {/* LEFT: Upgrade shop */}
        <div className="w-full lg:w-72 bg-slate-900/80 border-r border-slate-800 p-4 flex flex-col gap-2 overflow-y-auto" style={{ maxHeight: '85vh' }}>
          <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-2">🛒 Upgrade Shop</p>
          {visibleUpgrades.length === 0 && (
            <p className="text-slate-600 text-xs">Produci più stimolazione per sbloccare gli upgrade!</p>
          )}
          {visibleUpgrades.map(u => (
            <button key={u.id + upgradeCount}
              onClick={() => buyUpgrade(u)}
              disabled={stim < u.cost}
              className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all text-xs ${
                stim >= u.cost
                  ? 'bg-slate-800 border-purple-500/40 hover:bg-purple-900/30 hover:border-purple-400 cursor-pointer active:scale-95'
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-400 text-[10px]">{u.desc}</p>
                {(u.spc > 0 || u.sps > 0) && (
                  <p className="text-purple-400 text-[10px]">
                    {u.spc > 0 ? `+${u.spc} spc ` : ''}{u.sps > 0 ? `+${u.sps} sps` : ''}
                  </p>
                )}
              </div>
              <div className="text-right shrink-0">
                <p className="font-black text-yellow-400">{u.cost.toLocaleString()}</p>
                <p className="text-slate-500 text-[10px]">stim</p>
              </div>
            </button>
          ))}
          ...
          {hasAchieve && (
            <div className="mt-4">
              <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-2">🏆 Achievements ({achievements.length}/{ACHIEVEMENTS_DEF.length})</p>
              {ACHIEVEMENTS_DEF.map(a => (
                <div key={a.id} className={`flex items-center gap-2 py-1 text-xs ${achievements.includes(a.id) ? 'text-yellow-400' : 'text-slate-700'}`}>
                  <span>{achievements.includes(a.id) ? '✅' : '⬜'}</span>
        ...
        {/* CENTER: Main game */}
        <div className="flex-1 flex flex-col items-center justify-center gap-6 py-10 px-4 relative">

          {/* Score */}
          <div className="text-center">
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-1">Stimolazione</p>
            <p className="font-black text-6xl sm:text-7xl text-purple-300" style={{ textShadow: sps >= 20 ? '0 0 30px rgba(168,85,247,0.6)' : undefined }}>
              {Math.floor(stim).toLocaleString()}
            </p>
            {hasSpsCounter && (
              <p className="text-slate-400 text-sm mt-1">
                {sps.toFixed(1)} stim/sec · {spc} stim/click
              </p>
            )}
            <p className="text-slate-600 text-xs mt-1">Totale prodotta: {Math.floor(totalProd).toLocaleString()}</p>
          </div>
          ...
          <motion.button
            whileTap={{ scale: 0.92 }}
            onClick={handleClick}
            className={`relative font-black text-white shadow-2xl cursor-pointer flex flex-col items-center justify-center gap-2 transition-all ${
              has('btn_upgrade') ? 'w-48 h-48 rounded-[2.5rem] text-xl' : 'w-36 h-36 rounded-2xl text-lg'
            }`}
            style={{
              background: uiUpgrade
                ? 'linear-gradient(135deg, #7c3aed, #db2777, #7c3aed)'
                : 'linear-gradient(135deg, #7c3aed, #4f46e5)',
              boxShadow: `0 0 ${30 + sps}px rgba(124,58,237,0.5)`,
            }}
          >
            <span className="text-5xl">
              {sps >= 100 ? '🤯' : sps >= 35 ? '😵‍💫' : sps >= 10 ? '🧠' : '👆'}
            </span>
            <span className="text-sm font-black tracking-wide">CLICK ME</span>
            {has('btn_upgrade') && <span className="text-xs opacity-60">+{spc} stim</span>}
          </motion.button>

          {/* Subway Surfers panel */}
          {hasSubway && (
            <div className="bg-black rounded-2xl border border-slate-700 overflow-hidden w-48 h-28 flex items-center justify-center relative">
              <div className="absolute inset-0 bg-gradient-to-b from-sky-900 to-emerald-900" />
              <motion.div
                animate={{ x: [-60, 60, -60] }}
              ...
              <motion.div
                ...
                className="absolute bottom-3 text-2xl z-10"
              >🚇</motion.div>
              <p className="absolute top-1 right-2 text-[10px] text-white/40 font-bold z-10">Subway Surfers</p>
            </div>
          )}

          {has('lofi') && (
            <div className="flex items-center gap-3 bg-slate-800/60 border border-slate-700 rounded-xl px-4 py-2">
              <motion.span animate={{ rotate: [0, 360] }} transition={{ repeat: Infinity, duration: 2, ease: 'linear' }} className="text-2xl">🎵</motion.span>
              <div>
                <p className="text-slate-200 text-xs font-bold">Lofi Beats – Tokyo Rain</p>
                <p className="text-slate-500 text-[10px]">Lofi Girl · +{sps} sps</p>
              </div>
            </div>
          )}
          ...
          {has('rain') && (
            <div className="flex items-center gap-2 text-blue-400 text-sm">
              <motion.span animate={{ y: [0, 4, 0] }} transition={{ repeat: Infinity, duration: 0.8 }}>🌧️</motion.span>
              <span className="text-xs text-slate-500">Pioggia ASMR attiva</span>
            </div>
          )}

          {has('hydraulic') && (
            <div className="flex items-center gap-2 text-slate-400 text-xs">
              <span className={`text-2xl transition-transform ${hydraulicSquish ? 'scale-y-50' : 'scale-y-100'}`}>🔩</span>
              <span>Hydraulic Press – prossimo squish in 8s (+50 stim)</span>
            </div>
          )}
        </div>
      </div>
